...
export default function EditMusicVideo() {
  ...
  const [videoData, setVideoData] = useState<MusicVideo | null>(null);
  ...
  const fetchVideoData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${CONFIG.API_URL}/api/music-videos/${id}`,
        getAuthHeaders()
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const videoUrlToUse = videoUrl || formData.video_url || videoData.video_url;

      // Use uploaded thumbnail URL if available, otherwise use manual input, or keep existing thumbnail
      const thumbnailUrlToUse = thumbnailUrl || formData.thumbnail || videoData.thumbnail || null;
      ...
      const response = await axios.put(
        `${CONFIG.API_URL}/api/music-videos/${videoData.id}`,
        payload,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      error("Failed to Update Video", msg);
      toast.error(msg);
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="p-6">
            <div className="flex items-center justify-center min-h-[400px]">
              <div className="text-center">
                ...
                <p className="text-gray-600">Memuat data video...</p>
              </div>
            </div>
          </div>
        </Layout>
  ...
  if (!videoData) {
    return (
      <>
        <Head>
          <title>Video Not Found - SoundCave</title>
        </Head>
        <Layout>
          <div className="p-6">
          ...
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Update music video information
              </p>
            </div>
            <button
              onClick={() => router.back()}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <form onSubmit={handleSubmit}>
              ...
              <div className="mt-6 flex items-center justify-end space-x-3">
                ...
                >
                  {isLoading ? "Updating..." : "Update Music Video"}
                </button>
              </div>
            </form>
          </div>
