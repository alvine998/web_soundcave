...
export default function Podcasts() {
  ...
  const fetchPodcasts = async (pageParam = 1, query?: string) => {
    try {
      setIsLoading(true);
      ...
      if (response.data?.success && response.data?.data) {
        const podcastsList = response.data.data.map((item: any) => ({
          id: item.id,
          title: item.title,
          host: item.host,
        ...
        const uniqueCategories = Array.from(
          new Set(podcastsList.map((p: Podcast) => p.category))
        ) as string[];
      ...
      error("Failed to Fetch Podcasts", msg);
      toast.error(msg);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    fetchPodcasts(page, searchQuery);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page]);
  ...
  const handleEditClick = (podcast: Podcast) => {
    router.push(`/podcasts/edit/${podcast.id}`);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${CONFIG.API_URL}/api/podcasts/${selectedPodcast.id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: token ? `Bearer ${token}` : "",
      ...
      if (response.data?.success) {
        ...
        throw new Error(response.data?.message || "Failed to delete podcast");
      }
    } catch (err: any) {
      const msg =
        err?.response?.data?.message ||
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                {podcasts.map((podcast) => (
                  ...
                  >
                    {/* Thumbnail */}
                    <div className="h-48 bg-gradient-to-br from-purple-400 to-indigo-600 flex items-center justify-center overflow-hidden">
                      {podcast.thumbnail ? (
                        <img
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        {podcast.season && podcast.episode_number && (
